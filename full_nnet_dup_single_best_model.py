...
# create model
    ...
    model.add(Dense(512, input_dim = x_train.shape[1], kernel_initializer = 'he_normal'))
    model.add(PReLU())
...
model = KerasClassifier(build_fn=create_model, epochs=90, batch_size=50, verbose=1)
...
sub['high'] = pred_nn[:, 0]
sub.to_csv("model_nn_10.csv", index = False, header = True)
