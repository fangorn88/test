...
# create model
    ...
    model.add(Dense(175, input_dim=x_train.shape[1], kernel_initializer='normal', activation='relu'))
...
sub['high'] = pred_nn[:, 0]
sub.to_csv("model_nn_5.csv", index = False, header = True)

print(results.mean())


#pred_nn = model.predict(x_test)
#submission = pd.DataFrame(pred_nn, columns = ['high','medium','low']).set_index(test_df.index)
#submission['listing_id'] = test_df.listing_id.values
#submission.to_csv('model_nn_1.csv', index= False)
