...
# create model
    ...
    model.add(Dense(256, input_dim = x_train.shape[1], kernel_initializer = 'he_normal',kernel_regularizer=regularizers.l2(.00001)))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(Dropout(0.5))
        
#    model.add(Dense(128, kernel_initializer = 'he_normal',kernel_regularizer=regularizers.l2(.00001)))
#    model.add(PReLU())
#    model.add(BatchNormalization())    
#    model.add(Dropout(0.35))
    
    model.add(Dense(64, kernel_initializer = 'he_normal',kernel_regularizer=regularizers.l2(.0001)))
    model.add(PReLU())
    model.add(BatchNormalization())    
...
model = KerasClassifier(build_fn=create_model, epochs=70, batch_size=50, verbose=1)
...
sub['high'] = pred_nn[:, 0]
sub.to_csv("model_nn_21.csv", index = False, header = True)
