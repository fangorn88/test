...
x_train = np.loadtxt("stacknet/train_mx.csv", delimiter=",")
x_test = np.loadtxt("stacknet/test_mx.csv", delimiter=",")

print x_train.shape
print x_test.shape


#model = RandomForestClassifier(n_estimators=300, criterion='entropy', max_depth=4, max_features= 0.7, min_samples_split =2               #                                ,bootstrap=False,n_jobs=-1, oob_score = False, random_state=3, verbose=1)
...
#    model.add(Dense(512, input_dim = x_train.shape[1], kernel_initializer = 'he_normal',kernel_regularizer=regularizers.l2(.0001)))
    model.add(Dense(15, input_dim = x_train.shape[1], kernel_initializer = 'he_normal',activation='relu'))
#    model.add(PReLU())
...
np.savetxt("stnet/train_stacker_meta2_nn.csv", train_stacker, delimiter=",", fmt='%.6f')
...
np.savetxt("stnet/test_stacker_meta2_nn.csv", pred_nn, delimiter=",", fmt='%.6f')
