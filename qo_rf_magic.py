...
model = RandomForestClassifier(n_estimators=300, criterion='entropy', max_depth=15,max_features= 0.35, min_samples_split =10,                                            min_samples_leaf = 2   ,bootstrap = True,n_jobs=12, oob_score = True, random_state=2016, verbose=1)
...
np.savetxt("train_stacker_rf2.csv", train_stacker, delimiter=",", fmt='%.6f')

print 'finished out of fold predictions on training set'             

np.savetxt("test_stacker_rf2.csv", np.array(b['sum']), delimiter=",", fmt='%.6f')
